...
class Exporter
{
     ...
     * the bank.
     *
     * @param PaymentData $pd Payment data to send as encoded lines.
     *
    ...
    private function encodeOpening($from_bankgiro, $creation_date, $payment_date)
    {
        return sprintf(
            ...
            $from_bankgiro,
            $creation_date,
            utf8_decode("LEVERANTÖRSBETALNINGAR"), // XXX: This is the only valid value for this field.
            $payment_date,
            "",
            "",
            "",
    ...
    private function encodePaymentPost($payment)
    {
        if ($payment->is_deposit) {
            return sprintf(
                "14%09d %-25.25s%012d%6s%5s%-20.20s%s",
                $payment->payment_number,
                $this->encode($payment->reference),
                $payment->amount,
                $payment->payment_date,
                "",
                $this->encode($payment->sender_reference),
                $this->newLineChars
            );
        }
        return sprintf(
            "14%010d%-25.25s%012d%6s%5s%-20.20s%s",
            $payment->bankgiro,
            $this->encode($payment->reference),
            $payment->amount,
            $payment->payment_date,
    ...
    {
        return sprintf(
            "40%04d%05d %4d%012d%-12.12s%1s%39s%s",
            0,
    ...
    {
        return sprintf(
            "54%010d%-25.25s%012d%6s%5s%-20.20s%s",
            $payment->plusgiro,
            $this->encode($payment->reference),
            $payment->amount,
            $payment->payment_date,
